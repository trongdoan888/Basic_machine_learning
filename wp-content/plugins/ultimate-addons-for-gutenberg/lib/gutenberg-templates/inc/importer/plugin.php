...
class Plugin {
	...
	public function import_sureforms() {
		...
		if ( empty( $sureforms_url ) ) {
			wp_send_json_error( __( 'Empty SureForms URL', 'ultimate-addons-for-gutenberg' ) );
		}
	...
	public function get_page_palette_colors() {
		...
		$color_palettes = array(
			'style-1' =>
				array(
					'slug'          => 'style-1',
					'title'         => __( 'Light', 'ultimate-addons-for-gutenberg' ),
					'default_color' => $default_palette_color[ $reorganize ? 5 : 4 ],
					'colors'        => $palette_one,
				),
			'style-2' => array(
				'slug'          => 'style-2',
				'title'         => __( 'Dark', 'ultimate-addons-for-gutenberg' ),

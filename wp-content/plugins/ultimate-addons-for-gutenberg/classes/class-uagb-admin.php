...
if ( ! class_exists( 'UAGB_Admin' ) ) {
	...
	final class UAGB_Admin {
		...
		private function prepare_rollback_environment() {
			...
			update_option( 'uagb_rollback_in_progress', 'yes' );

			// Store rollback initiation timestamp for safety cleanup.
			update_option( 'uagb_rollback_timestamp', time() );

			// Clear beta update transient to prevent contamination.
			$beta_transient_key = md5( 'uagb_beta_testers_response_key' );
		...
		public function check_stale_rollback_state() {
			...
			if ( ! is_admin() || 'yes' !== get_option( 'uagb_rollback_in_progress', 'no' ) ) {
				return;
			}
			...
			if ( 0 === $rollback_timestamp || time() - $rollback_timestamp > HOUR_IN_SECONDS ) {
				// UAGB Admin: Cleaning up stale rollback state detected on admin_init.
				...
				// UAGB Admin: Stale rollback state cleanup completed, beta updates re-enabled.
			}
		}
		...
		public function post_upgrade_cleanup( $upgrader_object, $options ) {
			...
			if ( 'yes' === get_option( 'uagb_rollback_in_progress', 'no' ) ) {
				...
				// UAGB Admin: Post-upgrade rollback state cleanup completed.
			}
		}

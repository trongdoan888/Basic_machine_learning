...
if ( ! class_exists( 'UAGB_Beta_Updates' ) ) {
	...
	final class UAGB_Beta_Updates {
		...
		public function check_version( $transient ) {
			...
			// Skip beta updates if rollback is in progress.
			if ( 'yes' === get_option( 'uagb_rollback_in_progress', 'no' ) ) {
				// Safety cleanup: Clear stale rollback state if older than 1 hour.
				$rollback_timestamp = get_option( 'uagb_rollback_timestamp', 0 );
				if ( $rollback_timestamp > 0 && time() - $rollback_timestamp > HOUR_IN_SECONDS ) {
					// UAGB Beta Updates: Cleaning up stale rollback state.
					delete_option( 'uagb_rollback_in_progress' );
					delete_option( 'uagb_rollback_timestamp' );
					
					// Also clean up forced rollback flag from transient.
					$update_plugins_cleanup = get_site_transient( 'update_plugins' );
					if ( is_object( $update_plugins_cleanup ) && isset( $update_plugins_cleanup->uagb_forced_rollback ) ) {
						unset( $update_plugins_cleanup->uagb_forced_rollback );
						set_site_transient( 'update_plugins', $update_plugins_cleanup );
					}
					
					// UAGB Beta Updates: Stale rollback state cleanup completed.
				} else {
					// UAGB Beta Updates: Skipping beta check due to rollback in progress.
					return $transient;
				}
			}

			// Additional safety check: Skip if a forced rollback is active.
			$update_plugins = get_site_transient( 'update_plugins' );
			if ( is_object( $update_plugins ) && isset( $update_plugins->uagb_forced_rollback ) ) {
				// UAGB Beta Updates: Skipping beta check due to active forced rollback.
				return $transient;
			}
